...
public class SupplierPortImpl implements SupplierPortType {
	...
	public List<ProductView> searchProducts(String descText) throws BadText_Exception {
		...
		List<ProductView> foundProducts;
		...
		existingProducts = listProducts();
		foundProducts = new ArrayList<ProductView>();

		for (ProductView pv : existingProducts) {
			if (pv.getDesc().contains(descText)) {
				foundProducts.add(pv);
			}
		}
	...
	public String buyProduct(String productId, int quantity) throws BadProductId_Exception, BadQuantity_Exception, InsufficientQuantity_Exception {
		...
		if (productId == null) {
			...
			if (product == null) {
				throwBadProductId("The product that you tryed to purchase does not exist!");
			} else if (quantity < 1) {
				throwBadQuantity("Quantity needs to be equal or superior to one!");
			} else {
				purchaseId = supplier.buyProduct(productId, quantity);
				return purchaseId;
			}
		} catch (QuantityException qE) {
			throwInsufficientQuantity(qE.getMessage());
		}
		return "";
	}
